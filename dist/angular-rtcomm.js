...
angular.module('angular-rtcomm-ui').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/rtcomm/rtcomm-chat.html',
    "<div><div class=\"panel panel-primary vertical-stretch\"><div class=\"panel-heading\"><span class=\"glyphicon glyphicon-comment\"></span> Chat</div><div class=\"panel-body\"><ul class=\"chat\"><li ng-if=\"!(chatVM.chatActiveEndpointUUID == null)\" class=\"right clearfix\" ng-repeat=\"chat in chatVM.chats\"><div id=\"{{$index}}\" class=\"header\"><strong class=\"primary-font\">{{chat.name}}</strong> <small class=\"pull-right text-muted\">{{chat.time | date:'HH:mm:ss'}}</small></div><p>{{chat.message.text}}</p></li><li ng-if=\"(chatVM.chatActiveEndpointUUID == null)\" class=\"right clearfix\"><p>Usuario no conectado o aún no ha aceptado la tutoría</p></li></ul></div><div class=\"panel-footer\"><div class=\"input-group\"><input id=\"chat-input\" type=\"text\" class=\"form-control input-sm\" placeholder=\"Escribe tu mensaje aquí...\" type=\"text\" ng-model=\"chatVM.message\" ng-keypress=\"chatVM.keySendMessage($event)\"> <span class=\"input-group-btn\"><button class=\"btn btn-primary btn-sm\" id=\"btn-chat\" ng-click=\"chatVM.sendMessage()\" focusinput=\"true\" ng-disabled=\"(chatVM.chatActiveEndpointUUID == null)\">Enviar</button></span></div></div></div></div><!-- chat list ng-controller div -->"
  );
